...
@section('content')
<div class="py-12 bg-slate-50" x-data="{ role: '{{ old('role', 'seeker') }}' }">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <button type="button" @click="role = 'seeker'"
                            :class="role === 'seeker' ? 'border-emerald-600 bg-emerald-50/70 text-emerald-900 ring-2 ring-emerald-500' : 'border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100'"
                        ...
                        <p class="text-[11px] text-slate-500">I am looking for a spouse for myself (Groom or Bride).</p>
                    </button>

                    <button type="button" @click="role = 'wali'"
                            :class="role === 'wali' ? 'border-purple-600 bg-purple-50/70 text-purple-900 ring-2 ring-purple-500' : 'border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100'"
                            class="p-4 rounded-xl border text-left transition flex flex-col justify-between">
                        <div class="flex items-center gap-2 mb-1">
                            <span class="text-lg">🛡️</span>
                            <span class="font-bold text-sm font-heading">Guardian / Wali</span>
                        </div>
                        <p class="text-[11px] text-slate-500">I am registering on behalf of my daughter, sister, or ward.</p>
                    </button>
                </div>
            </div>
            ...
            <form action="{{ route('register') }}" method="POST" class="space-y-4">
                ...
                <div x-show="role === 'seeker'">
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Marital Status</label>
                    <select name="marital_status" :required="role === 'seeker'" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none bg-white">
                ...
                <div x-show="role === 'wali'" style="display: none;">
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Relationship to Candidate / Ward</label>
                    <select name="relationship_type" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none bg-white">
                ...
                <div class="pt-2">
                    <label class="flex items-start gap-2.5 text-xs text-slate-600">
                        ...
                        <span>
                            I affirm that I am using this platform strictly for marriage (Nikah), I agree to conduct interactions with Islamic etiquette, and I accept the <a href="#" class="text-emerald-700 underline font-medium">Terms of Service</a> and <a href="#" class="text-emerald-700 underline font-medium">Privacy Policy</a>.
                        </span>
                    </label>
                </div>
            ...
            <div class="text-center mt-6 pt-4 text-xs text-slate-500 border-t border-slate-100">
                Already registered? <a href="{{ route('login') }}" class="font-bold text-emerald-700 hover:text-emerald-800">Sign in here</a>
            </div>
        </div>
    </div>
</div>
