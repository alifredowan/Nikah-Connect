    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 shadow-xs p-6">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            <h1 class="text-xl font-extrabold text-slate-900 font-heading">{{ $user->name }}</h1>
                            @if($user->is_verified)
            ...
            <div class="pt-6">
                <div class="flex items-center justify-between text-xs mb-1.5">
                    <span class="font-bold text-slate-700">Profile Completeness (FR-2.4)</span>
                    <span class="font-extrabold text-emerald-700">{{ $profile->completeness_percentage }}%</span>
                </div>
                ...
                <p class="text-[11px] text-slate-400 mt-1">Complete your religious and family preferences to appear higher in daily compatibility recommendations.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-xs space-y-4">
            <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                <div>
                    <h2 class="text-base font-bold text-slate-900 font-heading">My Profile Photos ({{ $profile->photos->count() }}/6)</h2>
                    <p class="text-xs text-slate-500">Photos remain blurred to candidates by default for modesty until mutual approval (FR-2.2).</p>
                </div>
            </div>

            <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
                @foreach($profile->photos as $photo)
                    <div class="relative group rounded-2xl overflow-hidden border border-slate-200 aspect-square bg-slate-800">
                ...
                @if($profile->photos->count() < 6)
                    <!-- Upload Photo Card -->
                    <form action="{{ route('profile.photos.upload') }}" method="POST" enctype="multipart/form-data" class="border-2 border-dashed border-slate-300 rounded-2xl aspect-square flex flex-col items-center justify-center p-3 text-center hover:bg-slate-50 transition cursor-pointer relative">
                        @csrf
                        ...
                        <span class="text-[9px] text-slate-400 mt-0.5">Max 5MB</span>
                    </form>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-xs">
            <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <h2 class="text-base font-bold text-slate-900 font-heading">Government ID & KYC Verification (FR-1.4)</h2>
                        <p class="text-xs text-slate-500">Earn the trusted "Verified" badge to build authenticity and peace of mind.</p>
                    </div>
                </div>
                @if($user->is_verified)
            ...
            @if(!$user->is_verified)
                <form action="{{ route('profile.verification.submit') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                    @csrf
                    <div>
                        <label class="block font-bold text-slate-700 uppercase tracking-wider mb-1">Document Type</label>
                        <select name="document_type" required class="w-full p-2.5 rounded-xl border border-slate-300 outline-none bg-white">
                    ...
                    <div>
                        <label class="block font-bold text-slate-700 uppercase tracking-wider mb-1">Upload Document (Front)</label>
                        <input type="file" name="document" required accept=".jpg,.jpeg,.png,.pdf"
                               class="w-full p-2 rounded-xl border border-slate-300 outline-none bg-white text-slate-600">
                    </div>
                    <div>
                        <label class="block font-bold text-slate-700 uppercase tracking-wider mb-1">Selfie Verification</label>
            ...
            @else
                <p class="text-xs text-slate-600">Alhamdulillah, your identity documents have been verified by our moderation team.</p>
            @endif
        </div>

        <!-- Account Deactivation & GDPR Right to Erasure (FR-1.6) -->
        <div class="bg-rose-50/60 border border-rose-200 rounded-3xl p-6">
            <h3 class="text-sm font-bold text-rose-900 font-heading mb-1">Deactivation & GDPR Right to Erasure (FR-1.6)</h3>
            <p class="text-xs text-rose-800/80 mb-4">You may deactivate your account or request complete data deletion at any time under GDPR privacy protections.</p>
            <form action="{{ route('account.deactivate') }}" method="POST" onsubmit="return confirm('Are you sure you want to deactivate your profile?');" class="flex flex-col sm:flex-row gap-3">
                ...
                <input type="text" name="reason" required placeholder="Reason for deactivation (e.g. Found spouse alhamdulillah, taking a break)"
                       class="flex-1 px-4 py-2 rounded-xl border border-rose-300 text-xs focus:ring-2 focus:ring-rose-500 outline-none bg-white">
