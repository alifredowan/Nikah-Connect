...
<div class="py-10 bg-slate-50 min-h-screen">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-extrabold text-slate-900 font-heading">Edit Islamic & Personal Profile</h1>
                <p class="text-xs text-slate-500 mt-1">Detailed Islamic preferences help our algorithm rank compatible candidates for you (FR-2.1, 6.4).</p>
            </div>
            <a href="{{ route('profile.show') }}" class="text-xs font-semibold text-slate-500 hover:text-emerald-700">
                Cancel & Return
            </a>
        </div>
        ...
        <form action="{{ route('profile.update') }}" method="POST" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-xs space-y-4">
                <div class="flex items-center gap-2 pb-3 border-b border-slate-100">
                    ...
                    <h2 class="text-base font-bold text-slate-900 font-heading">Islamic Practice & Values (Highest Compatibility Weight)</h2>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Sect / Madhhab</label>
                        <select name="sect_madhhab" class="w-full px-3 py-2.5 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Daily Prayer (Salah) Frequency</label>
                        <select name="prayer_frequency" class="w-full px-3 py-2.5 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                    ...
                    @if($user->gender === 'female')
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Hijab / Modesty Practice</label>
                            <select name="hijab_niqab_practice" class="w-full px-3 py-2.5 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                    ...
                    @else
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Sunnah Beard Practice</label>
                            <select name="beard_practice" class="w-full px-3 py-2.5 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                                <option value="">Select Practice</option>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Quran Knowledge</label>
                        <select name="quran_knowledge" class="w-full px-3 py-2.5 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Halal Dietary Adherence</label>
                        <select name="halal_dietary_adherence" class="w-full px-3 py-2.5 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Mosque Attendance</label>
                        <select name="mosque_attendance" class="w-full px-3 py-2.5 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                ...
                <div class="pt-4 border-t border-slate-100">
                    <label class="flex items-start gap-3 text-xs text-slate-700 cursor-pointer">
                        ...
                        <div>
                            <strong class="text-slate-900 block font-bold">🛡️ Require Guardian (Wali) Involvement (FR-4.3)</strong>
                            <span class="text-slate-500">When enabled, suitors cannot unlock direct private messaging with you until your linked guardian confirms approval.</span>
                        </div>
                    </label>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-xs space-y-4">
                <div class="flex items-center gap-2 pb-3 border-b border-slate-100">
                    <span class="text-xl">🎓</span>
                    <h2 class="text-base font-bold text-slate-900 font-heading">Personal, Education & Profession</h2>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Education Level</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Profession</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Bio / Sincere Marriage Intentions</label>
                    <textarea name="bio" rows="4" class="w-full p-3 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="Describe your religious commitment, family background, and what you look for in a blessed marriage...">{{ old('bio', $profile->bio) }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-xs space-y-4">
                <div class="flex items-center gap-2 pb-3 border-b border-slate-100">
                    <span class="text-xl">🏡</span>
                    <h2 class="text-base font-bold text-slate-900 font-heading">Family Background</h2>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Family Religiosity</label>
                        <select name="family_religiosity" class="w-full px-3 py-2.5 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Desired Living Structure</label>
                        <select name="desired_family_structure" class="w-full px-3 py-2.5 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Number of Siblings</label>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-xs space-y-4">
                <div class="flex items-center gap-2 pb-3 border-b border-slate-100">
                    <span class="text-xl">🎯</span>
                    <h2 class="text-base font-bold text-slate-900 font-heading">Partner Preferences (Algorithmic Targeting)</h2>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Preferred Age Range</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Preferred Location</label>
